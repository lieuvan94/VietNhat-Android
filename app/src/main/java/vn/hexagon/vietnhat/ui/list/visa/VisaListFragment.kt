...
import kotlinx.android.synthetic.main.fragment_visa_list.*
...
import vn.hexagon.vietnhat.databinding.FragmentVisaListBinding
 ...
 * =====================================================
 */
class VisaListFragment: MVVMBaseFragment<FragmentVisaListBinding, PostListViewModel>(), DialogSearchClickListener {
    // View model
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        leftActionBarButton.setOnClickListener {
            findNavController().popBackStack(R.id.visaListFragment, true)
        }
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = CommonSearchDialogFragment(Constant.VISA_SERVICE_NM)
                dialog.setTargetFragment(this@VisaListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_visa_list

    override fun initView() {
        // init adapter
        val visaAdapter = VisaListAdapter(postListViewModel, ::onItemClick, ::onClickFav)
        // Refresh layout
        visaListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
        activity?.let { context ->
            visaRecyclerView.apply {
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            visaAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse, content:String) {
        visaListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = VisaListFragmentDirections.actionVisaListFragmentToPostDetailFragment(userId, postId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(visaListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}