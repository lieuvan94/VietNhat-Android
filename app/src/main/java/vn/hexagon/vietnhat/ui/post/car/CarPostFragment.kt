package vn.hexagon.vietnhat.ui.post.car
...
import kotlinx.android.synthetic.main.fragment_post_car.*
...
import vn.hexagon.vietnhat.databinding.FragmentPostCarBinding
 ...
 */
class CarPostFragment: MVVMBaseFragment<FragmentPostCarBinding, ProductPostViewModel>(), View.OnClickListener,
    ...
    override fun initData(argument: Bundle?) {
        ...
        argument?.let {
            mode = CarPostFragmentArgs.fromBundle(it).mode
            postId = CarPostFragmentArgs.fromBundle(it).postId
    ...
    override fun getLayoutId(): Int = R.layout.fragment_post_car
    ...
    override fun initView() {
        ...
        initRecycler()
        // Add products
        carProductPlaceHoldItem.setOnClickListener(this)
        // Address picker
        carPostEdAddress.setOnClickListener(this)
        // Commit data
        carPostCommitBtn.setOnClickListener(this)
        // Select cover image
        carPostSelectImg.setOnClickListener(this)
    }
    ...
    private fun getDetailResponse(response: PostDetailResponse?) {
        if (response?.errorId == Constant.RESPOND_CD) {
            ...
            initImageHorizontal()
            carPostEdAddress.setText(response.data.address)
            carPostEdContent.setText(response.data.content)
            carPostEdMessenger.setText(response.data.note)
            carPostEdPhone.setText(WindyConvertUtil.getPhoneNumber(response.data.phone))
            val countryCode = WindyConvertUtil.getStringBetween(response.data.phone, "+"," ").toInt()
            carCountryCodePicker.setCountryForPhoneCode(countryCode)
            carPostEdTitle.setText(response.data.title)
            carPostPinCheck.isChecked = response.data.isTop == Constant.TOP_CHECKED
     ...
     * @param response
     */
    private fun getProductListResponse(response: ProductResponse?) {
        if (response?.errorId == Constant.RESPOND_CD) {
            carProductLbl.visibility = if (response.data.isNotEmpty()) View.VISIBLE else View.GONE
            // Add products init
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.carPostEdAddress -> mPlaces?.displayPlacePicker(this, Constant.REQUEST_CD_PLACE)
            R.id.carPostSelectImg -> addImages()
            R.id.carProductPlaceHoldItem -> {
                fragmentManager?.let {
            ...
            R.id.carPostCommitBtn -> processSendDataPost()
        }
    }
            ...
            when(mode) {
                Constant.MODE_CREATE -> {
                    productPostViewModel.sendDataPost(
                        userId,
                        Constant.CAR_SERVICE_ID,
                        WindyConvertUtil.convertPhoneNumber(carCountryCodePicker, carPostEdPhone),
                        if (carPostPinCheck.isChecked) Constant.TOP_CHECKED else Constant.TOP_UNCHECK,
    ...
    private fun processSendDataPost() {
        // Validate
        val arrayFields = arrayOf<EditText>(carPostEdTitle, carPostEdPhone, carPostEdAddress, carPostEdContent, carPostEdMessenger)
        val validate = validateFieldsGlobal(arrayFields, carCountryCodePicker, carPostEdPhone, productPostViewModel.img, carPostArea)
        if (!validate) return
        // Check purchase
        if (carPostPinCheck.isChecked) {
            fragmentManager?.let {
    ...
    override fun requestProductData(
        ...
        carProductLbl.visibility = if (productPostAdapter.list.size > 0) View.VISIBLE else View.GONE
    }
    ...
    private fun ProductPostAdapter.processRemoveItem(position: Int) {
        ...
        this.apply {
            ...
            Handler().postDelayed({
                carItemRecyclerView.invalidateItemDecorations()
            }, 500)
        }
        carPostArea.layoutTransition
        Snackbar.make(carPostArea, getString(R.string.remove_product_message), Snackbar.LENGTH_SHORT).show()
    }
    ...
    private fun addImages() {
        if (mHorizontalAdapter.itemCount == mHorizontalAdapter.LIMIT_SIZE) {
            Snackbar.make(carPostArea, getString(R.string.limit_photo_message), Snackbar.LENGTH_SHORT).show()
            return
        }
        fragmentManager?.let {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            Constant.REQUEST_CD_PLACE -> mPlaces?.receivedAddressProcess(resultCode, data, carPostEdAddress)
