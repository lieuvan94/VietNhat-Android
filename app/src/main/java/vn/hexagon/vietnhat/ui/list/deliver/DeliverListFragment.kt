...
import kotlinx.android.synthetic.main.fragment_deliver_list.*
...
import vn.hexagon.vietnhat.databinding.FragmentDeliverListBinding
import vn.hexagon.vietnhat.ui.dialog.search.DeliverDialogFragment
import vn.hexagon.vietnhat.ui.dialog.search.DialogSearchDeliverListener
 ...
 * =====================================================
 */
class DeliverListFragment: MVVMBaseFragment<FragmentDeliverListBinding, PostListViewModel>(), DialogSearchDeliverListener {
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        leftActionBarButton.setOnClickListener {
            findNavController().popBackStack(R.id.deliverListFragment, true)
        }
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = DeliverDialogFragment()
                dialog.setTargetFragment(this@DeliverListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_deliver_list

    override fun initView() {
        // init adapter
        val deliverAdapter = DeliverListAdapter(context, postListViewModel,::onItemClick, ::onClickFav)
        // Refresh layout
        deliverListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
        activity?.let { context ->
            deliverRecyclerView.apply {
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            deliverAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse, content:String) {
        deliverListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
     ...
     * @param roadType
     */
    override fun requestDeliverSearch(
        title: String?,
        roadType: String?
    ) {
        userId?.let {
            postListViewModel.requestSearchDeliver(
                it,
                title,
                roadType,
                Constant.INDEX,
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = DeliverListFragmentDirections.actionDeliverListFragmentToDeliverDetailFragment(postId, userId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(deliverListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}