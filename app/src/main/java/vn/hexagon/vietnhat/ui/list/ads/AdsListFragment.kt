...
import vn.hexagon.vietnhat.databinding.FragmentAdsListBinding
 ...
 * =====================================================
 */
class AdsListFragment: MVVMBaseFragment<FragmentAdsListBinding, PostListViewModel>(),
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = CommonSearchDialogFragment(Constant.ADS_SERVICE_NM)
                dialog.setTargetFragment(this@AdsListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_ads_list

    override fun initView() {
        // init adapter
        val adsAdapter = AdsListAdapter(context, postListViewModel,::onItemClick, ::onClickFav)
        // Refresh layout
        adsListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            adsAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse, content:String) {
        adsListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
    ...
    /**
     * Receive search conditions from dialog
     *
     * @param title
     * @param address
     */
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = AdsListFragmentDirections.actionAdsListFragmentToPostDetailFragment(userId, postId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(adsListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}