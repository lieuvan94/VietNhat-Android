...
import kotlinx.android.synthetic.main.fragment_translator_list.*
 ...
 * =====================================================
 */
class TranslatorListFragment: MVVMBaseFragment<LayoutItemListServiceTranslatorBinding, PostListViewModel>(), DialogSearchClickListener {
    // View model
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        leftActionBarButton.setOnClickListener {
            findNavController().popBackStack(R.id.translatorListFragment, true)
        }
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = CommonSearchDialogFragment(Constant.TRANSLATOR_SERVICE_NM)
                dialog.setTargetFragment(this@TranslatorListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_translator_list

    override fun initView() {
        // init adapter
        val transAdapter =
            TranslatorListAdapter(postListViewModel, this@TranslatorListFragment::onItemClick, ::onClickFav)
        // Refresh layout
        translateListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
            ...
            translateRecyclerView.apply {
                ...
                addOnScrollListener(object: EndlessScrollingRecycler(linearLayoutManager) {
                    override fun onLoadMore(numberPost: Int) {
                        ...
                        loadData(numberPost * Constant.POST_PER_PAGE)
                    }
                })
            }
        }
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            transAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse) {
        translateListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = TranslatorListFragmentDirections.actionTranslatorListFragmentToPostDetailFragment(userId, postId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(translateListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}