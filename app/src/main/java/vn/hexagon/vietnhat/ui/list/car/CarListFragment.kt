package vn.hexagon.vietnhat.ui.list.car
...
import vn.hexagon.vietnhat.databinding.FragmentCarListBinding
 ...
 * =====================================================
 */
class CarListFragment: MVVMBaseFragment<FragmentCarListBinding, PostListViewModel>(), DialogSearchClickListener {
    // View model
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        leftActionBarButton.setOnClickListener {
            findNavController().popBackStack(R.id.carListFragment, true)
        }
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = CommonSearchDialogFragment(Constant.CAR_SERVICE_NM)
                dialog.setTargetFragment(this@CarListFragment, 0)
    ...
    override fun initView() {
        // init adapter
        val carAdapter = CarListAdapter(postListViewModel, ::onItemClick, ::onClickFav)
        // Refresh layout
        carListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
        activity?.let { context ->
            carRecyclerView.apply {
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            carAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse, content:String) {
        carListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = CarListFragmentDirections.actionCarListFragmentToPostDetailFragment(userId, postId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(carListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}