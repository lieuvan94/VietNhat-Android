package vn.hexagon.vietnhat.ui.list.spa
...
import kotlinx.android.synthetic.main.fragment_spa_list.*
...
import vn.hexagon.vietnhat.databinding.FragmentSpaListBinding
 ...
 * =====================================================
 */
class SpaListFragment: MVVMBaseFragment<FragmentSpaListBinding, PostListViewModel>(), DialogSearchClickListener {
    // View model
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = CommonSearchDialogFragment(Constant.SPA_SERVICE_NM)
                dialog.setTargetFragment(this@SpaListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_spa_list

    override fun initView() {
        // init adapter
        val spaAdapter = SpaListAdapter(postListViewModel,::onItemClick, ::onClickFav)
        // Refresh layout
        spaListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
        activity?.let { context ->
            spaRecyclerView.apply {
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            spaAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse) {
        spaListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = SpaListFragmentDirections.actionSpaListFragmentToPostDetailFragment(userId, postId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(spaListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}