package vn.hexagon.vietnhat.ui.list.travel
...
import vn.hexagon.vietnhat.databinding.FragmentTravelListBinding
 ...
 * =====================================================
 */
class TravelListFragment: MVVMBaseFragment<FragmentTravelListBinding, PostListViewModel>(), DialogSearchClickListener {
    ...
    override fun isShowActionBar(): View? = SimpleActionBar(activity).apply {
        ...
        rightActionBarButton.setOnClickListener {
            fragmentManager?.let {
                val dialog = CommonSearchDialogFragment(Constant.TRAVEL_SERVICE_NM)
                dialog.setTargetFragment(this@TravelListFragment, 0)
    ...
    override fun getLayoutId(): Int = R.layout.fragment_travel_list

    override fun initView() {
        // init adapter
        val travelAdapter = TravelListAdapter(context, postListViewModel,::onItemClick, ::onClickFav)
        // Refresh layout
        travelListRefresher.setOnRefreshListener {
            loadData(Constant.POST_PER_PAGE)
        }
        activity?.let { context ->
            travelRecyclerView.apply {
        ...
        postListViewModel.postListResponse.observe(this, Observer { response ->
            ...
            travelAdapter.submitList(response.data)
    ...
    private fun getResponse(response: ListPostResponse) {
        travelListRefresher.isRefreshing = false
        if (response.errorId == Constant.RESPOND_CD) {
    ...
    private fun onItemClick(userId:String, postId:String) {
        val action = TravelListFragmentDirections.actionTravelListFragmentToPostDetailFragment(userId, postId)
    ...
    private fun onClickFav(isAdded: Boolean) {
        ...
        Snackbar.make(travelListParent, message, Snackbar.LENGTH_SHORT).show()
    }
}