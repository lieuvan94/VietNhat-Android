 ...
 */
class RestaurantPostFragment: MVVMBaseFragment<FragmentPostRestaurantBinding, ProductPostViewModel>(), View.OnClickListener,
    ...
    private fun getProductResponse(response: ProductResponse?) {
        if (response?.errorId == Constant.RESPOND_CD) {
            restaurantProductLbl.visibility = if (response.data.isNotEmpty()) View.VISIBLE else View.GONE
            // Add products init
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.restaurantPostEdAddress -> mPlaces?.displayPlacePicker(this, Constant.REQUEST_CD_PLACE)
            ...
            R.id.restaurantPostSelectImg -> {
                if (mHorizontalAdapter.itemCount == mHorizontalAdapter.LIMIT_SIZE) {
                    Snackbar.make(restaurantPostArea, getString(R.string.limit_photo_message), Snackbar.LENGTH_SHORT).show()
                    return
                }
                fragmentManager?.let {
    ...
    private fun processSendDataPost() {
        // Validate
        val arrayFields = arrayOf<EditText>(restaurantPostEdTitle, restaurantPostEdPhone, restaurantPostEdAddress, restaurantPostEdContent, restaurantPostEdMessenger)
        val validate = validateFieldsGlobal(arrayFields, restaurantCountryCodePicker, restaurantPostEdPhone, productPostViewModel.img, restaurantPostArea)
        if (!validate) return
        // Check purchase
            ...
            when(mode) {
                Constant.MODE_CREATE -> {
                    productPostViewModel.sendDataPost(
                        ...
                        WindyConvertUtil.convertPhoneNumber(restaurantCountryCodePicker, restaurantPostEdPhone),
    ...
    override fun requestProductData(
        ...
        restaurantProductLbl.visibility = if (productPostAdapter.list.size > 0) View.VISIBLE else View.GONE
    }
    ...
    private fun ProductPostAdapter.processRemoveItem(position: Int) {
        ...
        Snackbar.make(restaurantPostArea, getString(R.string.remove_product_message), Snackbar.LENGTH_SHORT).show()
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            Constant.REQUEST_CD_PLACE -> mPlaces?.receivedAddressProcess(resultCode, data, restaurantPostEdAddress)
